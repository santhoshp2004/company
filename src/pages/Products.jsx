...
export default function Products() {
  ...
  // expandedProduct stores the name of the open card (accordion style)
  ...
  function toggleProduct(prod) {
    setExpandedProduct(prev => (prev?.name === prod.name ? null : prod));
  }
            ...
            {CATEGORIES.map(catKey => {
              ...
              return (
                <button
                  key={catKey}
                  type="button"
                  onClick={() => { setActiveCategory(catKey); setExpandedProduct(null); }}
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {/* Product grid — accordion detail expands below each card */}
              {products.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  {products.map((prod, i) => {
                    const isOpen = expandedProduct?.name === prod.name;
                    return (
                      <div key={prod.name} className="flex flex-col">
                        ...
                        <motion.button
                          ...
                          className={`w-full text-left group rounded-2xl border p-5 transition-all duration-200 ${
                            isOpen
                              ? isLight
                                ? 'border-blue-400 shadow-lg shadow-blue-100 rounded-b-none border-b-0'
                                : 'border-blue-500/60 shadow-lg rounded-b-none border-b-0'
                              : `hover:-translate-y-0.5 ${cardBase}`
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <motion.svg
                                ...
                                className={`w-4 h-4 flex-shrink-0 ${isLight ? 'text-slate-400' : 'text-gray-500'}`}
                          ...
                          <p className={`mt-3 text-xs font-semibold flex items-center gap-1 transition-colors ${isOpen ? 'text-blue-500' : 'text-slate-400 group-hover:text-blue-500'}`}>
                        ...
                        <AnimatePresence>
                          {isOpen && (
                            ...
                            >
                              <div className={`rounded-b-2xl border border-t-0 p-5 ${
                                isLight
                                  ? 'bg-blue-50/60 border-blue-400'
                                  : 'bg-blue-500/8 border-blue-500/60'
                              }`}>

                                {/* Full description */}
                                <p className={`text-sm leading-relaxed mb-5 ${subCls}`}>{prod.desc}</p>

                                {/* Features grid */}
                                <h4 className={`text-xs font-bold uppercase tracking-widest mb-3 ${isLight ? 'text-slate-500' : 'text-gray-500'}`}>
                                  Key Features
                                </h4>
                                <div className="grid grid-cols-2 gap-2 mb-5">
                                  {(prod.features || []).map(f => (
                                    <div key={f} className={`flex items-center gap-2 p-2.5 rounded-xl ${isLight ? 'bg-white border border-blue-100' : 'bg-white/5 border border-white/8'}`}>
                                ...
                                <div className="flex flex-wrap items-center gap-3">
                                  {(prod.stats || []).map(s => (
                                    <div key={s.l} className={`rounded-xl px-4 py-2.5 text-center ${isLight ? 'bg-white border border-blue-100' : 'bg-blue-500/10 border border-blue-500/20'}`}>
                                      <p className="text-lg font-black text-blue-500">{s.v}</p>
                                      <p className={`text-[10px] ${subCls}`}>{s.l}</p>
                                    </div>
                                  ))}
                                  <Link
                                    to="/support"
                                    className="ml-auto inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white transition-all hover:-translate-y-0.5"
