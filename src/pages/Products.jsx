...
const fadeUp = { hidden: { opacity: 0, y: 16 }, visible: (i) => ({ opacity: 1, y: 0, transition: { delay: i * 0.06, duration: 0.35 } }) };
...
export default function Products() {
  ...
  const cat = CATEGORY_META[activeCategory];
  ...
  return (
    <div className={`min-h-screen ${pageBg} pt-[62px]`}>
      ...
      <div className="max-w-7xl mx-auto flex" style={{ minHeight: 'calc(100vh - 130px)' }}>
        ...
        {/* ── Fixed-width sidebar ── */}
        <aside className={`w-56 flex-shrink-0 sticky top-[62px] self-start ${sidebarBg}`} style={{ height: 'calc(100vh - 130px)', overflowY: 'auto' }}>
          <div className="py-4">
              ...
              <motion.div key={activeCategory}
                ...
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl"
                    style={{ background: `linear-gradient(135deg,${getGrad(cat.color)})` }}>
                    {cat.icon}
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-2 gap-5">
                  {products.map((prod, i) => (
                    <motion.div key={prod.name} custom={i} variants={fadeUp} initial="hidden" animate="visible">
                      <button type="button" onClick={() => setSelectedProduct(prod)}
                ...
                <div className={`rounded-3xl border p-7 ${isLight ? 'bg-white border-slate-200' : 'bg-white/5 border-white/8'}`}>
                  ...
                  <div className="flex gap-3">
                    {selectedProduct.stats.map(s => (
                      <div key={s.l} className={`flex-1 max-w-[140px] rounded-2xl p-4 text-center ${isLight ? 'bg-blue-50 border border-blue-100' : 'bg-blue-500/10 border border-blue-500/20'}`}>
